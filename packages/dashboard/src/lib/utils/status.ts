...
export function statusColor(s: Status): { dot: string; text: string; bg: string } {
	switch (s) {
		...
		case 'offline':
			return { dot: 'bg-[var(--destructive)]', text: 'text-[var(--destructive)]', bg: 'bg-[var(--destructive)]/10' };
		case 'checking':
			return { dot: 'bg-[var(--warning)] animate-pulse', text: 'text-[var(--warning)]', bg: 'bg-[var(--warning)]/10' };
		default:
