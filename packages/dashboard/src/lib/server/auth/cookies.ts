 ...
 *
 * M2-WS3 (Kleppmann) implements the cookie layer:
 *
 *   SESSION COOKIE
 ...
 *     Path:     /
 *     Max-Age:  30 days (rolling — the SvelteKit hook touches the
 ...
 *   cookie. In production, Secure is always set; the deploy step
 *   MUST terminate TLS at the reverse proxy and serve the dashboard
 *   over HTTPS.
 */
...
// ---------------------------------------------------------------------------
// SvelteKit-style cookie API (subset)
// ---------------------------------------------------------------------------

/**
 * Minimal SvelteKit `Cookies` interface — just the methods we use.
 * Both the real SvelteKit `cookies` and our test cookie jar satisfy
 * this shape. `get` returns the raw value (matches SvelteKit).
 */
...
/**
 * Set the session cookie. `value` is the server-generated token
 * returned by `SessionStore.createSession()`. The cookie is
 * HttpOnly; JS cannot read it. `Max-Age` is set to the rolling TTL
 * so the browser aligns its eviction with the server's `expires_at`.
 */
...
/**
 * Set the CSRF cookie. `value` MUST equal the `csrfToken` bound to
 * the current session. The cookie is NOT HttpOnly — the SPA reads
 * it and echoes it in the `X-CSRF-Token` header.
 */
...
/**
 * Compare a candidate CSRF token to the canonical value using a
 * constant-time string comparison. The two values should be the
 * same length (both base64url 32-byte CSPRNG); a length mismatch
