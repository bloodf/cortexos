/**
 * Real Docker data — reads live containers from `docker ps`.
 *
 * M3 replacement for the M2 stub-data.ts. Uses `docker ps --format json`
 * (Docker 25.0+) and falls back to `docker ps` with a custom format
 * on older versions.
 */
import { execFile } from 'node:child_process';
import { promisify } from 'node:util';
import type { Container, ContainerId, ContainerFilter, DockerImage, DockerVolume } from './stub-data';
import {
  asContainerId,
...
// ---------------------------------------------------------------------------
// Actions — wrappers around docker CLI for form actions
// ---------------------------------------------------------------------------
