 ...
 * real `incus list` / image / network / pool queries from
 * `legacy/lib/incus/preflight.ts` (which still exists in the old
 * template as a reference).
 *
 * The mock returns:
 *   - name availability: `pass: true` unless the seed already has it
 *   - image: `pass: true` for the three known aliases
 *   - storage pool: `pass: true` for `default`, `nvme`, `hdd`
 *   - network bridge: `pass: true` for `incusbr0`
 *   - hermes secret (when enabled): pass: true in the mock
 */
...
// ---------------------------------------------------------------------------
// Provisioning progress — used by the wizard review step to render a
// simulated step list when the user clicks "Launch".
// ---------------------------------------------------------------------------

/**
 * Build a deterministic sequence of `ProgressStep`s for a fresh
 * instance launch. The wizard's review step renders this list with
 * one `pending` step at a time as the mock executor ticks through.
 * M3 swaps to the real progress channel from the orchestrator.
 */
...
let execNamedExecutor: MockExecNamedExecutor = async (op, args, name) => {
  switch (op) {
    ...
    case 'term.cat': {
      const path = typeof args.path === 'string' ? args.path : '/etc/hostname';
      return {
        stdout: `${name}\n`,
        stderr: '',
        exitCode: 0,
