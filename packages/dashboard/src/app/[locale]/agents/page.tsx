...
export default function AgentsPage() {
  ...
  const file = useMemo(() => active?.files.find((f) => f.path === activeFile) ?? active?.files[0], [active, activeFile]);
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-3 lg:grid-cols-[260px_240px_1fr] min-h-[60vh]">
        <Card className="elev-1 p-2 space-y-1">
          {agents.map((a) => (
            <button type="button" key={a.slug} onClick={() => { setActiveSlug(a.slug); setActiveFile(null); }} className={cn("w-full text-left rounded-md px-2 py-2 flex items-center gap-2 hover:bg-muted/40", active?.slug === a.slug && "bg-accent text-accent-foreground")}>
        ...
        <Card className="elev-1 p-2 space-y-0.5">
          ...
          {active?.files.map((f) => (
            <button type="button" key={f.path} onClick={() => setActiveFile(f.path)} className={cn("w-full text-left rounded px-2 py-1.5 text-sm flex items-center gap-2 hover:bg-muted/40 font-mono", file?.path === f.path && "bg-accent text-accent-foreground")}>
