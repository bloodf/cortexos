/**
 * POST /api/mail-guardian/[id]/approve — approve a mail review.
 *
 * Sets owner_decision='keep', resolved_at=NOW(), approver='dashboard',
...
export const POST = defineRoute({
	...
	handler: async ({ event }) => {
		...
		const updated = await updateMailReviewDecision(db, id, 'keep', 'dashboard');
		if (!updated) {
			throw notFoundError(`Review ${id} not found`, 'mail_review');
		}
