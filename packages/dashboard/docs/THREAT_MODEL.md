...
**Version:** 0.3 (M2 — pre-M2 gate)
**Owner:** Schneier (Security Reviewer)
**Status:** Draft for M2 review (supersedes v0.2)
**Last updated:** 2026-06-03
**Scope:** `packages/dashboard` (SvelteKit app, M1 rewrite of the v0.2-era Next.js scaffold) + the `cortex-dashboard` service + the `root-helper` Unix-socket service it talks to, on the supported deploy targets (Docker Compose stack on Ubuntu 24.04+, and bare-metal systemd on the same). The v0.2 file is preserved in git history at `7890e5a`.
...
| § | Topic | Owner | Status |
| --- | --- | --- | --- |
| 0.8 | Operating envelope — LAN-only, root-required, LAN trust boundary | Schneier | Added in v0.3 |
...
### 0.8 Operating envelope (v0.3 — read first)

This section is the single most important context for any reader of the rest of the document. If you read nothing else, read this.

- **This dashboard is an internal LAN-only control plane for managing the host it runs on.** It is not an internet-facing product. There is no "public" deployment mode; there is no anonymous user. The "user" is the operator at the keyboard, on the LAN, who has chosen to point a browser at the dashboard's Tailscale IP.
- **It legitimately requires root.** The whole point of the dashboard is to do things on the host that require root — Docker container lifecycle, Incus instance lifecycle, `systemctl` actions on critical units, terminal sessions, env-browser reads of `/opt/cortexos/...` and `/opt/cortexos/.secrets/`. The root-helper Unix-socket service exists precisely because the dashboard process needs to escalate to root for these operations. Treating the requirement to run as root as a "finding" is a category error: the privilege is the product.
- **The trust boundary is the LAN.** The network attack surface is **"LAN compromise"** (a device on the same Tailscale network is hostile or compromised), not **"internet exposure"** (anyone on the public internet can reach us). This distinction is decisive for which controls are real and which are theatre.
- **v0.2 controls that stay valid under this envelope:**
  - **SR-020 / SR-051 (the bash-c ban) and the PB-1..PB-6 privileged-action controls** remain correct and necessary. They protect against **misconfiguration and injection**, not external attackers. A LAN-side attacker who can talk to the dashboard is *already* past the network boundary; the allowlist, the `requireAdmin` gate, the typed-phrase + token approval flow, and the audit log are what stop them from doing damage. These controls are the *only* thing standing between "LAN compromise" and "host compromise". They are the moat.
  - **Lints #1 / #2** (`child_process.exec` / `execSync` with user input, `eval(` in `app/api/**`) are enforced because they are the *cheapest* way to keep the `bash -c` ban from regressing under refactor. They are not a network defence.
  - **Cosign verification, dep pinning, OSV-scanner, Renovate non-auto-merge** (SR-151) stay valid. A compromised dependency in a LAN-only tool is still a compromised dependency. The threat here is supply-chain, not network.
  - **Audit log integrity (SR-090, SR-092, SR-094), secret handling (SR-070..SR-074), CSP / cookie hygiene (SR-001, SR-141)** — all stay valid for the same reason: the threat they defend against is internal actors, misconfiguration, and a compromised local account, not an external attacker bypassing the LAN.
- **v0.2 control that shifts in scope under this envelope:**
  - **SR-200 (per-IP rate limiting on unauth endpoints)** shifts its *purpose*, not its *mechanism*. In v0.2 it was framed as "must survive internet-scale brute force" on `/api/auth` and `/api/approvals/request`. Under the v0.3 envelope the relevant adversary is **not** an internet-scale brute-forcer; it is **accidental loops from internal callers** (a UI bug that POSTs in a tight loop, a cron that double-fires, a misbehaving test harness) and **LAN-side compromised clients** generating token-mint spam. The in-process limiter is still the right control; the per-IP threshold can be relaxed (the v0.2 thresholds of `>30 req/min` for `/api/approvals/request` and `>100 req/min` for `/api/auth` are still safe and still required, but they do not need to grow to "10k req/sec" as the v0.2 text implied). Prometheus observability (`cortex_dashboard_unauth_rate_limited_total{path=...}`) stays required; that is how we *notice* the accidental loop in the first place.
- **What this section does NOT change:**
  - The STRIDE table in §1, the SR-xxx list in §2, the approval flows in §3, the allowlist in §4, the audit requirements in §6, the test checklist in §8, the deferred items in §11, the gate criteria in §12, and the traceability matrix in §13 are unchanged in *content*. v0.3 is a documentation pass that adds this envelope and the v0.2 sweep items, not a re-derivation of the threat model.
  - The fact that the dashboard requires root is not a free pass to skip review of every privileged endpoint. PB-1..PB-6 exist precisely *because* the product is privileged; the privileged-action controls are the review.
- **What this section enables the reader to push back on:**
  - Reviewers should NOT re-add network-defence controls (fail2ban on the dashboard port, geofencing, external WAF, public-internet TLS-only deploy mode) — they do not match the threat model.
  - Reviewers should NOT relitigate the "must run as root" decision at every review. The decision is in this section; it is the product. Future reviews assume it.
  - Reviewers SHOULD keep pushing on injection, allowlist completeness, audit-log integrity, confirmation-token TTL, and supply-chain hygiene. Those are where LAN compromise becomes host compromise.
...
| 7 | I | CSRF: forged `Origin` header → 403; double-submit CSRF token missing on state-changing POST → 403 (both halves of SR-004) | integration (both halves) | SR-004 | Bash |
...
| 28 | I | Cookie attributes on `/api/auth` response: `HttpOnly; Secure; SameSite=Lax; Path=/; __Host-` prefix; assertions on *all* five attributes; missing `__Host-` in production env → 403 | integration (response-header assertion) | SR-001, T-001 | Bash |
...
| 34 | I | CSP header includes `script-src 'self'`; no `'unsafe-inline'`; the `dangerouslySetInnerHTML` ban in the terminal output path is covered by test #3 (this row covers the header half of SR-141 only) | integration (header assertion); #3 covers the lint half | SR-141, T-141 | Bash |
...
| 56 | U | AI tool call to `read_file` with arg `path=$(bash -c id)` → rejected at arg-validation step (schema-level), not just at policy.json class check. **Type boundary:** this test is intentionally at the *arg-validation* layer; the policy-class check is covered separately by test #24 (SR-100). The two tests together assert that injection attempts fail at *both* layers, not just the outer one. | unit (table-driven) — arg validation only; #24 covers policy class | SR-100, T-104 | Karpathy |
| 57 | I + E | Unauth endpoint rate limit: 101st `/api/auth` request from same IP in 60s → 429 with `Retry-After`; 31st `/api/approvals/request` POST from same IP in 60s → 429 (stricter limit, token-mint is higher-value target). **Type boundary:** this row mixes the in-process limiter unit behaviour (asserted via a table of (path, ip, n) → 429) with the E2E behaviour that the *handler* actually applies the limiter under real network conditions. The unit half is the limiter's contract; the E2E half proves the handler wires the limiter in. Both are required — passing the unit half does not prove the handler is wired. | unit (table-driven per-IP flood) + E2E (handler under real fetch) | SR-200, T-200 | Bash + Margaret |
...
61 tests. 53 in M0 gate. 2 v1.1-touched (#50 v1+component, #55 v1.1-only). The "8" in §11 refers to 8 *deferred items* (D-01..D-08), of which 4 are v1.1-deferred (D-01, D-02, D-03, D-08) and 4 are M0-decision / out-of-scope / accepted-risk / optional (D-04, D-05, D-06, D-07). v0.3 wording fix: do not conflate "deferred items" with "v1.1-deferred tests".
...
- **M0-gate threats covered:** 49 of 57 (the 4 v1.1-deferred or partial: T-071 v1.1 component, T-091 v1.1-only, T-102 v1.1-only, T-121 M0 with v1 mitigation; per-threat status in §13). The "8" claim in v0.2 was a misread of the §11 deferred-items count; it is corrected here.
...
| 0.2 | 2026-06-03 | Schneier | v0.2 patch: 8 verifier findings closed. (1) §10.3 false claim replaced with truthful statement about test-path coverage. (2) SR-051 row added (Incus shell → exec-named allowlist rename). (3) T-001/T-004/T-141 broken cross-refs fixed (point to working §2.2 SR-xxx). (4) 5 numerical self-inconsistencies fixed (lines, STRIDE rows, requirements, M0-gate). (5) T-200 + SR-200 + E2E-S-26/S-27 added (rate-limit DoS on `/api/auth` and `/api/approvals/request`). (6) §0.4 sandbox-runner scope pinned. (7) Test #56 + T-104 + SR-100 arg-smuggling row added. (8) T-031/T-032/T-041/T-052 weak §13 test refs replaced with concrete test IDs (#58-#61). Final counts: 1286 lines, 17 surfaces, 57 STRIDE rows, 53 requirements (49 M0 + 4 v1.1), 61 tests (53 M0 + 2 v1.1-touched — see v0.3 wording-fix note), 27 E2E scenarios, 8 deferred items in §11 (of which 4 are v1.1-deferred). |
| 0.3 | 2026-06-03 | Schneier | v0.3 M2 patch (4 substantive changes; no STRIDE/SR re-derivation). (1) **§0.8 Operating envelope** added near the top: explicit statement that the dashboard is a LAN-only internal control plane that legitimately requires root, that the trust boundary is "LAN compromise" not "internet exposure", and that v0.2 controls (SR-020/SR-051 bash-c ban + lints #1/#2 + PB-1..PB-6 privileged-action gates + audit-log integrity + SR-001/SR-141 cookie/CSP hygiene + SR-151 supply-chain) all stay valid because they defend against injection / misconfiguration / internal-actor, not external attackers. SR-200 rate limiting re-scoped: still required, still Prometheus-observable, but the *purpose* shifts from "survive internet-scale brute force" to "catch accidental loops from internal callers" (and LAN-side compromised clients token-mint-spamming). (2) **SR-001 / SR-004 / SR-141 text-vs-hint alignment sweep:** tests #28, #7, #34 had Type "U" in the §8.2 table but the Pass criterion said "integration"; fixed Type to "I" and enriched the test descriptions (test #7 now explicitly covers both halves of SR-004: forged Origin + missing double-submit token; test #28 now asserts all five cookie attributes including `__Host-` prefix; test #34 now notes the lint half is covered by test #3). (3) **"8 v1.1-deferred" wording fix** in §8.2 closing line and §13.1: the v0.2 text conflated "8 deferred items in §11" with "8 v1.1-deferred tests", which is wrong. Corrected to "2 v1.1-touched tests (#50, #55); 8 items in §11 = 4 v1.1-deferred (D-01, D-02, D-03, D-08) + 4 M0-decision/out-of-scope/accepted-risk/optional (D-04, D-05, D-06, D-07)". (4) **Test #56 (U) / #57 (I) type boundaries** clarified: test #56 now explicitly notes it is at the *arg-validation* layer (with #24 covering the policy-class layer — together they assert injection fails at *both* layers, not just the outer one); test #57 type updated from "I" to "I + E" and the Pass criterion notes the unit half (limiter's contract) and the E2E half (handler wires the limiter in) are both required. Scope line updated to reflect the SvelteKit M1 rewrite of the v0.2-era Next.js scaffold. Final counts: ~1320 lines, 17 surfaces, 57 STRIDE rows, 53 requirements (49 M0 + 4 v1.1), 61 tests (53 M0 + 2 v1.1-touched), 27 E2E scenarios, 8 deferred items in §11. |

---

*End of document. v0.3 — ~1320 lines, 57 STRIDE rows, 49 M0-gate threats (of 57), 53 requirements (49 M0 + 4 v1.1), 61 tests (53 M0 + 2 v1.1-touched), 27 E2E scenarios, 8 deferred items in §11 (of which 4 are v1.1-deferred and 4 are M0-decision / out-of-scope / accepted-risk / optional). Added §0.8 Operating envelope, bumped version, swept v0.2 findings (SR-001/004/141 type alignment, "8 v1.1-deferred" wording, test #56/#57 type boundaries). Ready for Edsger's M2 review.*
