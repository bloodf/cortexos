...
| WP-50 | Security test suite (CUTOVER GATE) | done | `src/lib/api/__tests__/security-gate.test.ts` (42 assertions, `// @vitest-environment node`) drives the REAL `defineApiRoute` pipeline (the `(Request)=>Response` core `defineServerFn` delegates to) with crafted Requests + in-memory sessions / FakePAM / PGlite. ALL 8 properties PROVEN, no finding: (1) PAM login coarse failure — unknown-user ≡ bad-password in code path, message, AND timing (<10x ratio, no enumeration oracle); (2) session resolution — valid→ctx.user, stale/expired/unknown→dropped, stale cookie cleared, rolling-expiry touch; (3) CSRF double-submit + session-bound — stolen cookie alone AND cookie==header-but-not-session-bound both 403, safe methods exempt, no-session→401; (4) RBAC — admin route rejects non-admin (403), cortexos-admin only, sudo/wheel never grant admin; (5) approval tokens — single-use (replay→412), action-hash bound (wrong payload→412), session bound (cross-session→412, token NOT consumed), no-token→412; (6) env-reveal — masked by default (no cleartext in serialized response), PAM-verified grant→cleartext same session, no cross-session leak, grant expiry re-masks, admin+session gated; (7) rate limits — per-IP login 6th→429+Retry-After, per-user unlock 6th→429, per-user bucket isolates users; (8) audit HMAC chain — in-memory + DB (`appendAuditLog`/`verifyAuditLogChain`) tamper detection (chain_hash, prev_hash, payload_hash) reports the broken row. No gate weakened; no `src/server` internals touched. Verified: `vitest run src/server src/lib/api` → 34 files / 449 tests GREEN. |
