...
export function AuditPage() {
  ...
  const cols: Column<AuditEntry>[] = [
    { key: "created_at", header: "When", sort: (r) => r.created_at, cell: (r) => <span className="text-xs text-muted-foreground">{relativeTime(r.created_at)}</span> },
  ...
  return (
    <div className="space-y-5">
      <PageHeader icon={<ScrollText className="size-5" />} title={t.nav.audit}
      ...
      />
      <DataTable columns={cols} initialSort="created_at" initialSortDir="desc" server={{ queryKey: ["audit"], fetch: api.auditList }} />
