...
export function AdminAuditPage() {
  ...
  const columns: Column<AuditEntry>[] = [
    ...
    },
    {
      ...
      sort: (r) => r.decision,
      cell: (r) => (
        <Badge variant={r.decision === "allow" ? "default" : "destructive"} className="text-[10px]">
          {r.decision}
        </Badge>
      ),
    },
    {
      key: "reason",
      header: "Reason",
      cell: (r) => <span className="text-xs text-muted-foreground">{r.decision_reason}</span>,
    },
    {
      key: "hash",
      header: "Args hash",
      cell: (r) => (
        <code className="text-[10px] text-muted-foreground">{r.args_hash.slice(0, 10)}…</code>
      ),
    },
  ];
