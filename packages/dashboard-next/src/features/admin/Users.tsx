...
import type { PamUser } from "@/mocks/types";
...
export function AdminUsersPage() {
  ...
  const columns: Column<PamUser>[] = [
    ...
    { key: "groups", header: "Groups", cell: (r) => (
      <div className="flex flex-wrap gap-1">{r.groups.map((g) => <Badge key={g} variant="secondary" className="text-[10px]">{g}</Badge>)}</div>
    ) },
  ...
  return (
    <div className="space-y-5">
      ...
      <DataTable
        ...
        filterFn={(r, q) => r.username.toLowerCase().includes(q) || r.groups.some((g) => g.includes(q))}
      />
    </div>
  );
}
