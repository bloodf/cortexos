...
const mintApprovalCore: ApiRouteCore = defineApiRoute({
	...
	auth: "admin",
	input: z
		.object({ action: z.string().min(1).max(256), payload: z.record(z.string(), z.unknown()).default({}) })
		.strict(),
	surface: "approvals",
...
describe("audit chain verify (WP-02 verifier)", () => {
	...
	it("returns valid=true on an intact chain", async () => {
		await appendAuditLog(db, { eventType: "approvals.mint", source: "approvals", payload: { a: 1 } });
		await appendAuditLog(db, { eventType: "approvals.grant", source: "approvals", payload: { a: 2 } });
	...
	it("returns valid=false with brokenAt after a tampered row", async () => {
		...
		await db.execute(
			sql`UPDATE audit_log SET chain_hash = '${sql.raw("0".repeat(64))}'`,
		);
		const res = await verifyAuditLogChain(db);
