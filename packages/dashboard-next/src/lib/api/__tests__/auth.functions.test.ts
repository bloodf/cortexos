...
const loginCore: ApiRouteCore = defineApiRoute({ methods: [loginGateOptions.method], ...loginGateOptions });
const logoutCore: ApiRouteCore = defineApiRoute({ methods: [logoutGateOptions.method], ...logoutGateOptions });
...
describe("auth.login (auth: public)", () => {
	it("bad credentials → coarse auth error (no user-enumeration), no session, audited denied", async () => {
		pam.setFakeUser({ username: "alice", password: "correct-horse", groups: ["cortexos-admin", "cortexos-users"] });
