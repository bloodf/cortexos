...
export function TopBar({ collapsed, onToggleCollapse, onOpenMobile, onOpenPalette, onOpenHelp }: Props) {
  ...
  return (
    <header className="sticky top-0 z-30 h-14 bg-background/80 backdrop-blur border-b flex items-center gap-2 px-3 sm:px-5">
      ...
      <nav className="hidden md:flex items-center text-sm text-muted-foreground min-w-0">
        {crumbs.map((c, i) => (
          <span key={i} className="flex items-center min-w-0">
            ...
            {c.to ? <Link to={c.to} className="hover:text-foreground truncate">{c.label}</Link> : <span className="text-foreground font-medium truncate">{c.label}</span>}
          </span>
        ))}
      </nav>
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" size="icon" aria-label="Accent"><Palette className="size-4" /></Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="end" className="w-40">
      ...
      <Popover>
        <PopoverTrigger asChild>
          <Button variant="ghost" size="icon" className="relative" aria-label="Notifications">
            ...
            {unread > 0 && <span className="absolute top-1.5 right-1.5 size-2 rounded-full bg-[var(--destructive)]" />}
          </Button>
        </PopoverTrigger>
            ...
            {notifs.map((n) => (
              <li key={n.id} className="px-3 py-2.5 hover:bg-muted/40">
                <div className="flex items-start gap-2">
                  <span className={cn("size-1.5 rounded-full mt-1.5 shrink-0",
                    n.severity === "error" ? "bg-[var(--destructive)]" : n.severity === "warn" ? "bg-[var(--warning)]" : "bg-[var(--primary)]")} />
