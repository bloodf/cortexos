...
export function CommandPalette({ open, onOpenChange, onOpenHelp }: Props) {
  ...
  const actions: { id: string; label: string; icon: any; admin?: boolean; run: () => void }[] = [
    ...
    { id: "act-crash", label: "Simulate: crash random service", icon: AlertTriangle, admin: true, run: () => {
      const target = live.crashRandom();
      qc.invalidateQueries({ queryKey: ["services"] });
      toast.error(target ? `Simulated outage: ${target.name}` : "Nothing online to crash");
    } },
    { id: "act-heal", label: "Simulate: heal all services", icon: Heart, admin: true, run: () => {
      live.healAll();
      qc.invalidateQueries({ queryKey: ["services"] });
      toast.success("All services back online");
    } },
    { id: "act-restart-caddy", label: "Restart caddy.service", icon: Lock, admin: true, run: () => toast.success("Restart queued: caddy.service") },
    ...
    <CommandDialog open={open} onOpenChange={onOpenChange}>
      ...
      <CommandList className="max-h-[60vh]">
            ...
            {recent.map((r) => {
              ...
              return (
                <CommandItem key={r} onSelect={() => runNav(r)}>
                  <nav.icon className="size-4 mr-2" />{t.nav[nav.key]}
                </CommandItem>
              );
            })}
          </CommandGroup>
        ...
        <CommandGroup heading={t.palette.nav}>
          {navItems.map((it) => (
            <CommandItem key={it.to} onSelect={() => runNav(it.to)} value={`nav ${t.nav[it.key]} ${it.to}`}>
        ...
        <CommandGroup heading="Accent">
          {ACCENTS.map((a) => (
            <CommandItem key={a.id} onSelect={() => { setAccent(a.id); toast.success(`Accent: ${a.label}`); close(); }} value={`accent ${a.label}`}>
              <span className="size-3 rounded-full mr-2" style={{ background: a.color }} />
              <Palette className="size-3.5 mr-2 text-muted-foreground" />
              {a.label}
              {accent === a.id && <span className="ml-auto text-[10px] text-muted-foreground">current</span>}
            </CommandItem>
          ))}
        </CommandGroup>
        ...
        <CommandGroup heading="Language">
          {LOCALES.map((l) => (
            <CommandItem key={l} onSelect={() => { setLocale(l as Locale); toast.success(`Language: ${LOCALE_LABEL[l]}`); close(); }} value={`lang ${LOCALE_LABEL[l]}`}>
              <Globe className="size-3.5 mr-2 text-muted-foreground" />
              {LOCALE_LABEL[l]}
              {locale === l && <span className="ml-auto text-[10px] text-muted-foreground">current</span>}
            </CommandItem>
          ))}
        </CommandGroup>
        ...
        <CommandGroup heading={t.palette.services}>
          {services.slice(0, 20).map((s) => (
            <CommandItem key={s.slug} onSelect={() => { window.open(s.open_url, "_blank"); close(); }} value={`svc ${s.name} ${s.slug} ${s.category}`}>
              <span className="size-2 rounded-full mr-2" style={{ background: s.icon_color ?? "var(--primary)" }} />
        ...
        <CommandGroup heading="Containers">
          {containers.slice(0, 15).map((c) => (
            <CommandItem key={c.id} onSelect={() => { runNav(`/docker/${c.id}`); }} value={`container ${c.name} ${c.image}`}>
              <span className="size-2 rounded-full mr-2" style={{ background: c.state === "running" ? "var(--success)" : "var(--muted-foreground)" }} />
              <span className="flex-1">{c.name}</span>
              <span className="text-[10px] text-muted-foreground truncate max-w-[160px]">{c.image}</span>
            </CommandItem>
          ))}
        </CommandGroup>
        ...
        <CommandGroup heading="Systemd units">
          {units.slice(0, 15).map((u) => (
            <CommandItem key={u.name} onSelect={() => { runNav(`/systemd/${u.name}`); }} value={`unit ${u.name} ${u.description}`}>
              <span className="size-2 rounded-full mr-2" style={{ background: u.active === "active" ? "var(--success)" : u.active === "failed" ? "var(--destructive)" : "var(--muted-foreground)" }} />
              <span className="flex-1 font-mono text-xs">{u.name}</span>
              <span className="text-[10px] text-muted-foreground truncate max-w-[160px]">{u.description}</span>
            </CommandItem>
          ))}
        </CommandGroup>
        ...
        {user?.is_admin && (
          <CommandGroup heading="Users">
            {users.slice(0, 10).map((u) => (
              <CommandItem key={u.username} onSelect={() => { runNav("/admin/users"); }} value={`user ${u.username} ${u.groups.join(" ")}`}>
        ...
        {user?.is_admin && (
          <CommandGroup heading="Recent audit">
            {audit.slice(0, 8).map((a) => (
              <CommandItem key={a.id} onSelect={() => { runNav("/audit"); }} value={`audit ${a.tool} ${a.actor} ${a.decision_reason}`}>
                <span className="size-2 rounded-full mr-2" style={{ background: a.decision === "allow" ? "var(--success)" : "var(--destructive)" }} />
      ...
      <div className="border-t px-3 py-1.5 flex justify-between text-[10px] text-muted-foreground">
        <span>{t.palette.hints.move}</span><span>{t.palette.hints.select}</span><span>{t.palette.hints.close}</span>
      </div>
    </CommandDialog>
  );
}
