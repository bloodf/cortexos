...
export class BotApiTelegramClient implements TelegramClient {
	...
	private async call<T>(method: string, body?: Record<string, unknown>): Promise<T> {
		...
		if (!payload.ok || payload.result === undefined) {
			throw new Error(`Telegram ${method} failed: ${payload.description ?? payload.error_code ?? "unknown error"}`);
		}
...
function telegramRequest<T>(token: string, method: string, body?: Record<string, unknown>): Promise<TelegramResponse<T>> {
