...
const baseArtifact: OmcArtifact = {
  ...
  runId: "R-1",
  steps: [
    { role: "planner", action: "plan", content: "Draft outline", timestamp: "2026-05-18T10:00:00Z" },
    { role: "executor", action: "edit", content: "const x = 1;\nconsole.log(x);", timestamp: "2026-05-18T10:05:00Z" },
  ],
  result: "All good",
};
...
describe("parseTranscript", () => {
  ...
  it("treats JSON-looking single-line strings as code", () => {
    const blocks = parseTranscript({
      ...baseArtifact,
      steps: [
        { role: "r", action: "a", content: '{"k":"v"}', timestamp: "t" },
      ],
      result: undefined,
    });
