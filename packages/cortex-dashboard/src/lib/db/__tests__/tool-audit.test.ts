...
import { insertAuditRow, listAudit } from '../tool-audit';
import * as auditModule from '../tool-audit';
...
describe('tool-audit (append-only)', () => {
  ...
  it('insertAuditRow inserts full row', async () => {
    ...
    expect(queryOne).toHaveBeenCalledWith(
      expect.stringContaining('INSERT INTO tool_audit'),
      expect.any(Array),
    );
  });
