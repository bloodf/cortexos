...
export default function NetworkPage() {
  ...
  const interfaces = (net as any)?.interfaces ?? [];
  const totalRx = interfaces.reduce((a: number, i: any) => a + (i.rxKbps ?? 0), 0);
  const totalTx = interfaces.reduce((a: number, i: any) => a + (i.txKbps ?? 0), 0);
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-3 grid-cols-2 sm:grid-cols-4">
        ...
        <MetricCard label="Lifetime Rx" value={bytes(interfaces.reduce((a: number, i: any) => a + (i.rxBytesTotal ?? 0), 0))} />
        <MetricCard label="Lifetime Tx" value={bytes(interfaces.reduce((a: number, i: any) => a + (i.txBytesTotal ?? 0), 0))} />
      </div>
      <NetworkTopology />
