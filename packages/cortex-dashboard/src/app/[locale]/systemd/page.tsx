...
export default function SystemdPage() {
  ...
  const cols: Column<SystemdUnit>[] = [
    { key: "name", header: "Unit", sort: (r) => r.name, cell: (r) => <Link href={`/systemd/${r.name}`} className="font-mono text-xs hover:underline">{r.name}</Link> },
  ...
  return (
    <div className="space-y-5">
      <PageHeader icon={<Server className="size-5" />} title={"Systemd"} description={`${units.length} units · ${units.filter((u) => u.active === "active").length} active`} />
      <DataTable rows={units} columns={cols} loading={isLoading} initialSort="name" filterFn={(r, q) => r.name.includes(q) || r.description.toLowerCase().includes(q)} />
    </div>
  );
}
