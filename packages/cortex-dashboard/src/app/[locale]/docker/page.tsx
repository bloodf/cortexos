...
import { DetailDrawer, MockLogs, MockMetrics, MockEnv } from "@/components/sys-pilot/DetailDrawer";
...
import type { DockerContainer, DockerImage, DockerVolume } from "@/lib/types";
...
export default function DockerPage() {
  ...
  return (
    <div className="space-y-5">
      <PageHeader icon={<Container className="size-5" />} title={"Docker"}
      ...
      <Tabs defaultValue="containers" className="space-y-4">
        ...
        <TabsContent value="containers"><DataTable rows={containers} columns={containerCols} loading={lc} initialSort="name" filterFn={(r, q) => r.name.includes(q) || r.image.toLowerCase().includes(q)} /></TabsContent>
        <TabsContent value="images"><DataTable rows={images} columns={imageCols} loading={li} initialSort="repo" filterFn={(r, q) => r.repo.toLowerCase().includes(q) || r.tag.includes(q)} /></TabsContent>
        <TabsContent value="volumes"><DataTable rows={volumes} columns={volumeCols} loading={lv} initialSort="name" filterFn={(r, q) => r.name.includes(q)} /></TabsContent>
      </Tabs>
