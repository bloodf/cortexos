...
export function AgentFileViewer({ slug, files }: Props) {
	...
	const selectedFile = useMemo(() => files.find((file) => file.name === activeFile), [activeFile, files]);
	...
	return <div className="space-y-4"><div className="flex gap-2 overflow-x-auto"><div className="flex rounded-md border border-border p-0.5">{files.map((file) => <button key={file.name} type="button" onClick={() => { setActiveFile(file.name); setEditedContent(null); setSaveError(null); }} className={`shrink-0 rounded px-3 py-1.5 font-mono text-xs transition-colors ${activeFile === file.name ? "bg-secondary text-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}>{file.name}</button>)}</div></div><div className="overflow-hidden rounded-xl border border-border bg-card"><div className="flex items-center justify-between border-b border-border px-4 py-2"><span className="font-mono text-xs text-muted-foreground">{selectedFile?.path ?? activeFile}</span><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => { setEditedContent(null); setSaveError(null); }} disabled={!dirty || saving}>Reset</Button><Button size="sm" onClick={saveEdit} disabled={!dirty || saving}>{saving ? "Saving…" : "Save"}</Button></div></div>{saveError && <div className="border-b border-red-500/20 bg-red-500/10 px-4 py-2 text-xs text-red-400">{saveError}</div>}{isLoading ? <div className="p-4 text-sm text-muted-foreground">Loading…</div> : error ? <div className="p-4 text-sm text-red-400">Failed to load file content.</div> : <Textarea value={draft} onChange={(event) => { setEditedContent(event.target.value); }} className="min-h-[560px] resize-y rounded-none border-0 bg-transparent font-mono text-sm focus-visible:ring-0" spellCheck={false} />}</div></div>;
}
