...
| MP-028 | retire the four runtime-mock admin duplicates (docker/incus/systemd/audit) — last runtime mock imports in first-party source eliminated; typed DashNotification stub for TopBar bell; /admin/projects → /incus | done | 0151f6b (+7026711, c5148c2, 25433f9) | critic PASS cycle 3; gpt-5.5 diff PASS (0 findings); suite 619/619; screens 18/18 + 5/5 probes (run 24); live 200 |
| MG-SOAK | mail-guardian 2h journal soak (campaign residual) | adjudicated | — | 'Invalid JSON response' = 0 (PASS); 'IMAP socket closed' = hourly :32 server-side recycle on shared IMAP host, designed recovery verified every cycle — environmental OVERRULE, GATE-RESOLUTION 2026-06-12 |
