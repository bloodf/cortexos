...
async function dashboardServices() {
  ...
  if (!url) return [];
  const sql = "select coalesce(json_agg(json_build_object('slug',slug,'name',name,'kind',kind,'health_type',health_type,'health_url',health_url) order by slug),'[]'::json) from services";
