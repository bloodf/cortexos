...
backup_clickhouse() {
  name="cortex-langfuse-clickhouse"
  outdir="${WORK_DIR}/databases/clickhouse"
  tarball="${WORK_DIR}/databases/clickhouse_backup.tar.gz"
  if ! have docker; then warn "docker unavailable for $name"; return; fi
  if ! container_running "$name"; then warn "container not running: $name"; return; fi
  run mkdir -p "$outdir"
  if dry; then log "[dry-run] dump ClickHouse databases from $name to $tarball"; return; fi
  dbs="$(docker exec "$name" clickhouse-client --query "SHOW DATABASES" | grep -Ev '^(INFORMATION_SCHEMA|information_schema|system)$' || true)"
  if [ -z "$dbs" ]; then warn "no ClickHouse databases found"; return; fi
  while IFS= read -r db; do
    [ -n "$db" ] || continue
    docker exec "$name" clickhouse-client --database "$db" --query "SHOW TABLES" | while IFS= read -r table; do
      [ -n "$table" ] || continue
      docker exec "$name" clickhouse-client --database "$db" --query "SHOW CREATE TABLE \`$table\`" > "${outdir}/${db}.${table}.schema.sql" || warn "clickhouse schema failed: $db.$table"
      docker exec "$name" clickhouse-client --database "$db" --query "SELECT * FROM \`$table\` FORMAT Native" > "${outdir}/${db}.${table}.native" || warn "clickhouse data failed: $db.$table"
    done
  done <<EOF
$dbs
EOF
...
main_backup() {
  ...
  copy_path "${CORTEX_ROOT}/paperclip" "${WORK_DIR}/services/paperclip"
  ...
  copy_glob "${WORK_DIR}/configs/systemd" '/etc/systemd/system/cortex-*' '/etc/systemd/system/hermes-*' '/etc/systemd/system/9router*' '/etc/systemd/system/ollama*' '/etc/systemd/system/paperclip*'
  ...
  copy_from_container cortex-langfuse-minio /data "${WORK_DIR}/configs/monitoring/minio"
  copy_from_container cortex-grafana /var/lib/grafana "${WORK_DIR}/configs/monitoring/grafana"
  ...
  copy_glob "${WORK_DIR}/configs/monitoring/loki" '/etc/systemd/system/loki*' "${CORTEX_ROOT}/monitoring/loki" "${CORTEX_ROOT}/stacks/monitoring/loki"
