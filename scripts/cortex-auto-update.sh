...
preflight() {
  if dry; then log "[dry-run] preflight: require /mnt/hdd mount and backup <24h"; return; fi
  if ! mountpoint -q /mnt/hdd; then log "/mnt/hdd is not mounted"; exit 2; fi
...
smart_restarts() {
  ...
  changed node && { restart_if_loaded cortex-dashboard.service || true; restart_pattern 'hermes-profile@*.service' || true; restart_if_loaded paperclip.service || true; }
  ...
  if changed brew; then
    ...
    restart_if_loaded paperclip.service || true
