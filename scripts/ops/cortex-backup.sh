...
# Staging must be on a POSIX filesystem (symlinks/hardlinks). BACKUP_ROOT may be a
# CIFS/NAS share that cannot store symlinks, so only the final encrypted artifact is
# written there. Defaults to BACKUP_ROOT for the legacy local-disk layout.
STAGING_ROOT="${STAGING_ROOT:-/var/tmp/cortex-backup}"
...
# Stage and build the plaintext tar on local disk (STAGING_ROOT); only the
# age-encrypted result is placed on BACKUP_ROOT (which may be CIFS/NAS).
WORK_DIR="${STAGING_ROOT}/${STAMP}"
ARCHIVE="${STAGING_ROOT}/${STAMP}.tar.gz"
ENCRYPTED="${BACKUP_ROOT}/${STAMP}.tar.gz.age"
...
preflight() {
  if ! dry; then
    mkdir -p "$STAGING_ROOT" || { log "cannot create staging dir: $STAGING_ROOT"; exit 2; }
    mkdir -p "$BACKUP_ROOT"  || { log "backup dest unavailable (NAS mounted?): $BACKUP_ROOT"; exit 2; }
  fi
...
main_backup() {
  ...
  write_manifest
  if dry; then
    log "[dry-run] tar -C $STAGING_ROOT -czf $ARCHIVE $STAMP"
    log "[dry-run] age -r AGE_PUBKEY -o $ENCRYPTED $ARCHIVE"
    log "[dry-run] rm -rf $WORK_DIR $ARCHIVE"
  else
    tar -C "$STAGING_ROOT" -czf "$ARCHIVE" "$STAMP"
