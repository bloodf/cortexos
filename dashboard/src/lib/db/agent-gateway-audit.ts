...
export interface AgentGatewayAuditRow {
	id: number;
	ts: Date;
...
export async function insertAuditRow(
	...
	const row = await queryOne<AgentGatewayAuditRow>(
...
export async function listAudit(
	...
	vals.push(offset);
	const offsetParam = i;
	return query<AgentGatewayAuditRow>(
		`SELECT ${COLUMNS} FROM agent_gateway_audit ${where} ORDER BY ts DESC LIMIT $${limitParam} OFFSET $${offsetParam}`,
		vals,
	);
}
...
export async function countAudit(
	...
	const row = await queryOne<{ count: string }>(
		`SELECT COUNT(*)::text AS count FROM agent_gateway_audit ${where}`,
		vals,
	);
